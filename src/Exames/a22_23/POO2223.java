...
public class POO2223 {
    ...
    private static void alinea2(PrintStream out) {
         ...
         */
         ...
         try {
            Scanner sc = new Scanner(new File("C:\\Users\\Marti\\OneDrive - Universidade de Aveiro\\Universidade\\1ºano\\Programação Orientada a Objetos\\Praticas\\src\\Exames\\a22_23\\bookings.txt"));
            while (sc.hasNextLine()) {
                ...
                int taxId = Integer.parseInt(info[0]);
                String startDate = info[1];
                int duration = Integer.parseInt(info[2]);
                String type = info[3];
                SpaceType spaceType = SpaceType.valueOf(type.toUpperCase());
                String[] dims = info[4].split("x");
                int[] minDimensions = new int[dims.length];
                for (int i = 0; i < dims.length; i++) {
                    minDimensions[i] = Integer.parseInt(dims[i]);
                }
                LocalDate sD = LocalDate.parse(startDate);



                Client c = campingService.getClient(taxId);
                List<CampingSpace> disponiveis = campingService.findAvailableCampingSpaces(spaceType, sD, duration, minDimensions);
                if (disponiveis != null && !disponiveis.isEmpty()) {
                    CampingSpace cs = disponiveis.get(0);
                    campingService.bookCampingSpace(c, cs, sD, duration);
                }
            }
            sc.close();
